...
import { fetchPortfolioById, updatePortfolio } from '../../../../redux/actions/portfolioActions';
import {
...
export default function EditPortfolioItem() {
    ...
    const { currentPortfolio, loading } = useSelector((s) => s.portfolio);
    ...
    useEffect(() => {
        if (id) {
            setFetching(true);
            dispatch(fetchPortfolioById(id)).finally(() => setFetching(false));
        }
    }, [id, dispatch]);
    ...
    useEffect(() => {
        if (currentPortfolio) {
            setForm({
                ...
                excerpt: currentPortfolio.excerpt || '',
                description: currentPortfolio.description || '',
                clientName: currentPortfolio.clientName || '',
                category: currentPortfolio.category || 'Web Development',
                status: currentPortfolio.status || 'published',
                liveUrl: currentPortfolio.liveUrl || '',
                githubUrl: currentPortfolio.githubUrl || '',
                completionDate: fmtDate(currentPortfolio.completionDate),
                featured: currentPortfolio.featured || false,
                order: currentPortfolio.order ?? 0,
                technologies: currentPortfolio.technologies || [],
            });
            setImagePreview(currentPortfolio.thumbnail || '');
        }
    }, [currentPortfolio]);
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await dispatch(updatePortfolio({ id, formData: fd, token }));
        if (updatePortfolio.fulfilled.match(res)) {
            router.push('/admin/portfolio');
        } else {
            setError(res.payload || 'Failed to update item.');
        }
    };
        ...
        <AdminLayout>
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Edit Portfolio Item</h1>
                        <p className="text-slate-500 text-sm mt-0.5 line-clamp-1 max-w-xs">{currentPortfolio?.title}</p>
                    </div>
                </div>
                <Link href={`/admin/portfolio/${id}`} className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-orange-500 border border-slate-200 hover:border-orange-300 px-4 py-2 rounded-xl transition-all bg-white">
                    View Detail
                </Link>
            </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input value={form.clientName} onChange={(e) => set('clientName', e.target.value)} className={FIELD} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5"><Calendar size={13} className="inline mr-1" />Completion Date</label>
                            ...
                            <div>
                                ...
                                <input value={form.excerpt} onChange={(e) => set('excerpt', e.target.value)} className={FIELD} />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">Full Description</label>
                                <textarea value={form.description} onChange={(e) => set('description', e.target.value)} rows={6} className={`${FIELD} resize-none`} />
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input value={form.liveUrl} onChange={(e) => set('liveUrl', e.target.value)} className={FIELD} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5"><Github size={12} className="inline mr-1" />GitHub URL</label>
                                    <input value={form.githubUrl} onChange={(e) => set('githubUrl', e.target.value)} className={FIELD} />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                                ...
                                <div className={`border-2 border-dashed rounded-xl flex flex-col items-center justify-center text-center transition-all h-44 overflow-hidden ${imagePreview ? 'border-orange-300' : 'border-slate-200 bg-slate-50 hover:border-orange-300'}`}>
                                    {imagePreview ? (
                                        ...
                                        <>
                                            ...
                                            <p className="text-sm text-slate-500 font-medium">Click to replace</p>
                                        </>
                                    )}
                                </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" min={0} value={form.order} onChange={(e) => set('order', e.target.value)} className={FIELD} />
                            </div>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <Link href="/admin/portfolio" className="text-center text-sm text-slate-500 hover:text-slate-700 font-medium transition-colors">Cancel</Link>
                        </div>
                    </div>
                </div>
            </form>
