...
export default function CreateService() {
    ...
    const { loading } = useSelector((s) => s.service);

    const [form, setForm] = useState({
        title: '', shortDescription: '', description: '', icon: '',
        category: 'Web Development', status: 'active', pricingModel: 'custom',
        startingPrice: '', deliveryTime: '', featured: false, order: '',
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await dispatch(createService({ formData: fd, token }));
        if (createService.fulfilled.match(res)) {
            router.push('/admin/services');
        } else {
            setError(res.payload || 'Failed to create service.');
        }
    };
    ...
    const TagList = ({ items, onRemove }) => (
        items.length > 0 ? (
            <div className="flex flex-wrap gap-2 mt-2">
                {items.map(item => (
                    <span key={item} className="inline-flex items-center gap-1.5 bg-orange-50 text-orange-700 text-xs font-medium px-3 py-1.5 rounded-lg border border-orange-100">
                        {item}
                        <button type="button" onClick={() => onRemove(item)} className="text-orange-400 hover:text-orange-600"><X size={12} /></button>
                    </span>
                ))}
            </div>
        ) : null
        ...
        <AdminLayout>
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-0.5">Add a new service to your offerings.</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={6}
                                    placeholder="Detailed description of what this service includes, how it works, and what clients get..."
                        ...
                        {/* Technologies */}
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            <h2 className="font-bold text-slate-800 flex items-center gap-2"><Code2 size={16} className="text-orange-500" /> Technologies / Tools</h2>
                            <div className="flex gap-2">
                            ...
                            <TagList items={form.technologies} onRemove={t => removeItem('technologies', t)} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            ...
                            {imagePreview && (
                                <button type="button" onClick={() => { setImageFile(null); setImagePreview(''); }}
                                    className="w-full text-xs text-red-500 hover:text-red-600 flex items-center justify-center gap-1"><X size={12} /> Remove</button>
                            )}
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input value={form.startingPrice} onChange={e => set('startingPrice', e.target.value)}
                                    placeholder="e.g., ₹25,000 onwards"
                                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 transition-all" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">Sort Order</label>
                                <input type="number" min={0} value={form.order} onChange={e => set('order', e.target.value)} placeholder="0"
                                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 transition-all" />
                            </div>
