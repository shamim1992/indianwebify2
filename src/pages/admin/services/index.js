...
import { fetchServices, deleteService } from '../../../redux/actions/serviceActions';
import {
    PlusCircle, Search, Pencil, Trash2, Eye, ChevronLeft, ChevronRight,
    Server, Star, CheckCircle2, XCircle, Clock, Filter, RefreshCw, Zap, Image as ImageIcon
} from 'lucide-react';
...
const STATUS_CONFIG = {
    ...
    'coming-soon': { label: 'Coming Soon', classes: 'bg-violet-50 text-violet-600 border-violet-100' },
};

const PRICING_LABELS = { fixed: 'Fixed', hourly: 'Hourly', monthly: 'Monthly', custom: 'Custom' };

export default function ServiceManagement() {
    const dispatch = useDispatch();
    const { token } = useSelector((s) => s.auth);
    const { services, loading, total, pages } = useSelector((s) => s.service);
    ...
    const loadServices = useCallback(() => {
        dispatch(fetchServices({ page, limit: LIMIT, status: statusFilter }));
    ...
    const handleDelete = async (id) => {
        ...
        setDeletingId(id);
        await dispatch(deleteService({ id, token }));
        setDeletingId(null);
        loadServices();
    };
    ...
    const stats = [
        { label: 'Total Services', value: total, icon: Server, color: 'bg-orange-50 text-orange-600' },
        { label: 'Active', value: services.filter(s => s.status === 'active').length, icon: CheckCircle2, color: 'bg-emerald-50 text-emerald-600' },
        { label: 'Inactive', value: services.filter(s => s.status === 'inactive').length, icon: XCircle, color: 'bg-slate-100 text-slate-500' },
        { label: 'Featured', value: services.filter(s => s.featured).length, icon: Star, color: 'bg-amber-50 text-amber-600' },
    ];
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900">Service Management</h1>
                    <p className="text-slate-500 mt-1">Manage all your services — create, edit, and publish.</p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-24 text-slate-400 gap-3">
                        ...
                        <p className="font-medium text-slate-500">No services found</p>
                        <Link href="/admin/services/create" className="text-sm text-orange-500 hover:text-orange-600 font-semibold flex items-center gap-1">
                            <PlusCircle size={16} /> Create your first service
                        </Link>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100 text-left">
                                    ...
                                    <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Pricing</th>
                                    <th className="px-4 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider hidden lg:table-cell">Delivery</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((svc) => {
                                    ...
                                    return (
                                        <tr key={svc._id} className="hover:bg-slate-50/70 transition-colors group">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {svc.coverImage ? (
                                                        ...
                                                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-100 to-orange-50 flex items-center justify-center shrink-0 text-lg">
                                                            {svc.icon || <Zap size={16} className="text-orange-400" />}
                                                        </div>
                                                    )}
                                                    <div>
                                                        <div className="font-semibold text-slate-900 group-hover:text-orange-600 transition-colors line-clamp-1 max-w-[180px] flex items-center gap-1">
                                                            {svc.title}
                                                            {svc.featured && <Star size={11} className="text-amber-400 fill-amber-400 shrink-0" />}
                                                        </div>
                                                        {svc.shortDescription && (
                                                            <p className="text-xs text-slate-400 mt-0.5 line-clamp-1 max-w-[180px]">{svc.shortDescription}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-4 py-4 hidden md:table-cell">
                                                <span className="inline-flex text-xs text-slate-600 bg-slate-100 px-2.5 py-1 rounded-lg font-medium">
                                                    {svc.category || 'General'}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 hidden lg:table-cell text-slate-600 text-xs">
                                                {svc.startingPrice ? (
                                                    <span className="font-semibold text-slate-800">{svc.startingPrice}</span>
                                                ) : (
                                                    <span className="text-slate-400">{PRICING_LABELS[svc.pricingModel] || 'Custom'}</span>
                                                )}
                                            </td>
                                            <td className="px-4 py-4 hidden lg:table-cell text-slate-500 text-xs">
                                                {svc.deliveryTime ? (
                                                    <span className="inline-flex items-center gap-1"><Clock size={11} /> {svc.deliveryTime}</span>
                                                ) : '—'}
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <Link href={`/admin/services/${svc._id}`}
                                                    ...
                                                    <button onClick={() => handleDelete(svc._id)} disabled={deletingId === svc._id}
                                                        className="p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all disabled:opacity-50" title="Delete">
                                                        {deletingId === svc._id ? (
                ...
                {!loading && pages > 1 && (
                    <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                        <p className="text-sm text-slate-500">
                            Page <span className="font-semibold text-slate-700">{page}</span> of <span className="font-semibold text-slate-700">{pages}</span>
                        </p>
