...
import { fetchProjectById, updateProject } from '../../../../redux/actions/projectActions';
import {
...
export default function EditProject() {
    ...
    useEffect(() => {
        if (id) {
            setFetching(true);
            dispatch(fetchProjectById(id)).finally(() => setFetching(false));
        }
    }, [id, dispatch]);
    ...
    useEffect(() => {
        if (currentProject) {
            setForm({
                ...
                excerpt: currentProject.excerpt || '',
                clientName: currentProject.clientName || '',
                category: currentProject.category || 'Web Application',
                status: currentProject.status || 'planning',
                budget: currentProject.budget || '',
                projectUrl: currentProject.projectUrl || '',
                githubUrl: currentProject.githubUrl || '',
                startDate: fmt(currentProject.startDate),
                endDate: fmt(currentProject.endDate),
                featured: currentProject.featured || false,
                order: currentProject.order ?? 0,
                technologies: currentProject.technologies || [],
            });
            setImagePreview(currentProject.coverImage || '');
        }
    }, [currentProject]);
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await dispatch(updateProject({ id, formData: fd, token }));
        if (updateProject.fulfilled.match(res)) {
            router.push('/admin/project');
        } else {
            setError(res.payload || 'Failed to update project.');
        }
    };
            ...
            <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-0.5 line-clamp-1 max-w-xs">{currentProject?.title}</p>
                    </div>
                </div>
                <Link href={`/admin/project/${id}`}
                    className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-orange-500 border border-slate-200 hover:border-orange-300 px-4 py-2 rounded-xl transition-all bg-white">
                    View Detail
                </Link>
            </div>
                    ...
                    <div className="space-y-6">
                                ...
                                <div className={`border-2 border-dashed rounded-xl flex flex-col items-center justify-center text-center transition-all h-44 overflow-hidden ${imagePreview ? 'border-orange-300 bg-orange-50/50' : 'border-slate-200 bg-slate-50 hover:border-orange-300 hover:bg-orange-50/30'}`}>
                                    {imagePreview ? (
                                        ...
                                        <>
                                            ...
                                            <p className="text-sm text-slate-500 font-medium">Click to replace image</p>
                                        </>
                                    )}
                                </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5"><Calendar size={12} className="inline mr-1" />Start</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5"><Calendar size={12} className="inline mr-1" />End</label>
